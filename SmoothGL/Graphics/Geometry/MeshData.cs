...
namespace SmoothGL.Graphics
{
    ...
    public class MeshData
    {
        ...
        /// <summary>
        /// Creates a new triangle mesh data set from a number of vertices, where every triple of vertices defines exactly one triangle.
        ...
        private static Vector3[] NormalsFromPositions(Vector3[] positions)
        {
            ...
            {
                Vector3 triangleNormal = Vector3.Cross(positions[i + 1] - positions[i], positions[i + 2] - positions[i]).Normalized();
                for (int j = 0; j < 3; ++j)
        ...
        /// <summary>
        /// Creates a new triangle mesh data set from a number of vertices and optional indices. When indices are specified, every triple of indices
        ...
        /// <param name="selector">Selector function which creates a single vertex of type T from a position, normal and texture coordinate.</param>
        /// <returns>Array of vertices.</returns>
        public T[] GetVertices<T>(Func<Vector3, Vector3, Vector2, T> selector)
        {
            T[] result = new T[NumberOfVertices];
        ...
        {
            return GetVertices<VertexPositionNormalTexture>(VertexPositionNormalTextureSelector);
        }
        ...
        public VertexBuffer ToVertexBuffer<T>(Func<Vector3, Vector3, Vector2, T> selector, VertexDeclaration vertexDeclaration)
            where T : struct
        {
            VertexBuffer vertexBuffer = new VertexBuffer(NumberOfVertices, vertexDeclaration, BufferUsage.Static);
            vertexBuffer.SetData<T>(GetVertices<T>(selector));
            return vertexBuffer;
        }

        /// <summary>
        /// Creates a vertex buffer stored on the GPU from the mesh's vertices, defining a position, normal and texture coordinate each.
        /// </summary>
        /// <returns>Vertex buffer.</returns>
        public VertexBuffer ToVertexBuffer()
        {
            return ToVertexBuffer<VertexPositionNormalTexture>(VertexPositionNormalTextureSelector, VertexPositionNormalTexture.VertexDeclaration);
        }

        /// <summary>
        /// Creates an element buffer from the mesh's indices. The minimum required number of bits per index is determined automatically.
        /// </summary>
        ...
        {
            T[] flippedElements = (T[])elements.Clone();
