...
/// </summary>
public class Texture2D : Texture, IHotSwappable<Texture2D>
{
    /// <summary>
    ...
    public Texture2D(int width, int height, TextureColorFormat format, TextureFilterMode filterMode)
        : base(TextureTarget.Texture2D, filterMode)
    {
        Format = format;
        Width = width;
        Height = height;
        GL.TexImage2D(TextureTarget.Texture2D, 0, (PixelInternalFormat)format, width, height, 0, PixelFormat.Rgba, PixelType.Float, IntPtr.Zero);
    }
